...
allEqual [] = True
allEqual [x] = True
allEqual (x:xs) = if x /= head xs then False else allEqual xs
...
--the logic is correct but produces an error I don't understand
...
longest :: Ord a => [[a]] -> [a]
longest lists = case length longestLists of 1 -> head longestLists
                                            _ -> smallestHead
  where longestLists = filter (\x -> length x == maximum (map length lists)) lists
        smallestHead = head $ filter (\x -> head x == minimum (map head longestLists)) longestLists
...
incrementKey k kvs = map increment kvs
  where increment (x,v) = if x == k then (x,(v+1)) else (x,v)
...
winner scores player1 player2 = if fst t >= snd t then player1 else player2
  where t = ((Map.lookup player1 scores),(Map.lookup player2 scores))
...
freqs xs = foldr insert Map.empty xs
  where insert = (\ i -> Map.insertWith (+) i 1)
