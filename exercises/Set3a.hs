...
firstHalf :: [p] -> [p]
firstHalf p = take (length p - div (length p) 2) p
...
capitalize ws = unwords $ map (\w -> toUpper (head w) : tail w) $ words ws
...
whileRight check x = case check x of Right a -> whileRight check a
                                     Left b  -> b
...
joinToLength n strings = filter (\x -> length x == n) [ x++y | x <- strings, y <- strings ]
...
sumRights x = sum $ map (\(Right x) -> x) $ filter isRight x
...
interpreter [] = []
interpreter ("up":xs) = interpreter (xs <> ["U"])
interpreter ("down":xs) = interpreter (xs <> ["D"])
interpreter ("left":xs) = interpreter (xs <> ["L"])
interpreter ("right":xs) = interpreter (xs <> ["R"])
interpreter ("printY":xs) = showY xs ++ (interpreter xs)
  where
    showY l = [show $ (length $ filter (== "U") l) - (length $ filter (== "D") l)]
interpreter ("printX":xs) = showX xs ++ (interpreter xs)
  where
    showX l = [show $ (length $ filter (== "R") l) - (length $ filter (== "L") l)]
interpreter _ = []
