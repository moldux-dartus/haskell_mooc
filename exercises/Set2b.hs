...
count x = reverse (map (\x -> (show x) ++ "... ") [1..x]) 

countdown :: Integer -> String
countdown x = "Ready! " ++ foldl (++) "" (count x) ++ "Liftoff!"
...
smallestDivisor x = head $ [ y | y <- [2..x], mod x y == 0]
...
biggestPrimeAtMost n = head $ filter isPrime (reverse [1..n])
