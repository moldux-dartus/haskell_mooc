...
data KeyVals k v = {-Error String |-} Empty | Pair k v (KeyVals k v) deriving Show

toList :: KeyVals k v -> [(k,v)]
toList (Pair k v rest) = (k, v) : (toList rest)
toList Empty = []

fromList :: (Show k, Show v) => [(k,v)] -> KeyVals k v
fromList [] = Empty
fromList list{-@[(k,v)]-} = Pair a b (fromList $ tail list)
  where a = fst $ head list
        b = snd $ head list
--fromList x = Error $ show x
...
toNat :: Int -> Maybe Nat
toNat x = if (x < 0) then Nothing else Just $ makeNat x
  where makeNat 0 = Zero
  --      makeNat 1 = PlusOne Zero
        makeNat i = PlusOne (makeNat (i-1))
{-        
toNat 0 = Just Zero
toNat 1 = Just $ PlusOne Zero
toNat x
  | x < 0 = Nothing
  | otherwise = Just (PlusOne $ toNat x-1)
-}
